...
class resetPower:
	def POST(self):
		...
		for port in powerPort:
			try:
				print '[*] Turn off port '+str(port)
				sw.write(str(port)+'|0|3000')
				time.sleep(4)
			except Exception,e:
...
class downloadAll:
	def POST(self):
		...
		for dev in USBLIST:
			try:
				downloadCommand = ['xterm',"-T","osmocon for "+dev,'-e',sys.path[0]+'/osmocombb_x64/osmocon','-m','c123','-s','/tmp/osmocom_l2_'+ dev.split('USB')[1] ,'-p',dev,sys.path[0]+'/osmocombb_x64/layer1.compalram.bin']
...
class download:
	def POST(self):
		...
		try:
			downloadCommand = ['xterm',"-T","osmocon for "+deviceId,'-e',sys.path[0]+'/osmocombb_x64/osmocon','-m','c123','-s','/tmp/osmocom_l2_'+ deviceId.split('USB')[1] ,'-p',deviceId,sys.path[0]+'/osmocombb_x64/layer1.compalram.bin']
			downloadProcess = subprocess.Popen(downloadCommand,stderr=subprocess.PIPE,stdout=subprocess.PIPE)
			time.sleep(1)
		except Exception,e:
			return json.dumps({"res":-1,"msg":str(e)})
